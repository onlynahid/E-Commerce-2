...
namespace AYYUAZ.APP.Infrastructure.Services
{
    ...
    {
        public static async Task SeedInitialData(AppDbContext context, IServiceProvider serviceProvider)
        {
        ...
        private static async Task SeedAdminAsync(IServiceProvider serviceProvider)
        {
            ...
            if (adminUser == null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // Also add to User role (admin can also be a user)
                    await userManager.AddToRoleAsync(adminUser, "User");
                }
            }
            else
            {
                // Ensure existing admin user has IsAdmin = true and proper roles
